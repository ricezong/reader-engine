...
interface BaseSource : JsExtensions {
    ...
    fun evalJS(jsStr: String, bindingsConfig: SimpleBindings.() -> Unit = {}): Any? {
        ...
        // 预加载 jsLib（如果书源定义了自定义 JS 库函数）
        // 将 jsLib 和 jsStr 合并为一个脚本执行，确保 jsLib 中的函数定义在同一个 scope 中
        val jsLibCode = (this as? BookSource)?.jsLib
        val fullScript = if (!jsLibCode.isNullOrBlank()) {
            "$jsLibCode\n$jsStr"
        } else {
            jsStr
        }
